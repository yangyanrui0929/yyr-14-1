...
import type { Piece, Seal, CustomPuzzle } from '@/types';
import { ArrowLeft, Save, Play, Trash2, Check } from 'lucide-react';
import { deleteCustomPuzzle, getCustomPuzzles } from '@/utils/storage';
import { cn } from '@/lib/utils';

export function Editor() {
  const navigate = useNavigate();
  const { savePuzzle, pieces, seals, puzzleName, loadPuzzle } = useEditorStore();
  ...
  const [customPuzzles, setCustomPuzzles] = useState<CustomPuzzle[]>(getCustomPuzzles());
  ...
  const handleTest = () => {
    ...
    let puzzleId = savedPuzzleId;
    if (!puzzleId) {
      puzzleId = handleSave();
    }

    if (puzzleId) {
      navigate(`/game/custom-${puzzleId}`);
    }
  };
  ...
  const handleLoadPuzzle = (puzzle: CustomPuzzle) => {
    loadPuzzle(puzzle);
  ...
  const handleUpdatePiece = (piece: Piece) => {
    setSelectedPiece(piece);
  };

  const handleUpdateSeal = (seal: Seal) => {
              ...
              <div className="mt-4 bg-white/5 backdrop-blur-sm rounded-xl p-4 border border-white/10">
                ...
                <div className="space-y-2 max-h-60 overflow-y-auto">
                  {customPuzzles.map((puzzle) => (
                    <div
                      key={puzzle.id}
                      className="group flex items-center justify-between text-sm rounded-lg px-2 py-1.5 hover:bg-white/10 cursor-pointer transition-colors"
                      onClick={() => handleLoadPuzzle(puzzle)}
                    >
                      <span className="text-white/60 truncate flex-1 group-hover:text-white transition-colors">
                        {puzzle.name}
                      </span>
                      <button
                        onClick={(e) => {
                        ...
                        className="opacity-0 group-hover:opacity-100 text-white/30 hover:text-red-400 transition-all ml-2"
                      >
                        <Trash2 size={14} />
                      </button>
                    </div>
